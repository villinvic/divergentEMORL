...
import neural_mmo.forge.blade.core.config as mmo_config
import neural_mmo.forge.trinity.env as mmo
...
class Worker(Default):
    def __init__(self, render=False, hub_ip='127.0.0.1'):
        ...
        self.mmo = mmo.Env(mmo_config.Simple())
        self.game = Game(render)
        self.player = Individual(-1, self.game.state_dim, Action.n, [])


        self.tileWeight = np.array([1.0, 0.0, 0.02, 0.02], dtype=np.float32)
        self.entWeight = np.array([1.0, 0.0, 0.0, 0.05, 0.00, 0.02, 0.02, 0.1, 0.01, 0.1, 0.1, 0.1, 0.3], dtype=np.float32)
