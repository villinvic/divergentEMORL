...
class Hub(Default, Logger):
    def __init__(self, ip='127.0.0.1', ckpt=''):
        ...
        self.policy_distributions = np.zeros((self.pop_size+self.n_offspring, self.n_traj_ref, self.BATCH_SIZE,
                                              self.TRAJECTORY_LENGTH, dummy_env.action_dim), dtype=np.float32)
        self.perf_and_uniqueness = np.zeros((self.pop_size+self.n_offspring), dtype=np.float32)
    ...
    def make_offspring(self):
        ...
        parents_pairs = np.random.choice(self.pop_size, (self.n_offspring, 2), replace=False)
