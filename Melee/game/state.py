...
from Melee.game.state_manager import StateManager
from Melee.game.enums import ActionState
from Melee.game.memory_watcher import MemoryWatcherZMQ
from Melee.input.action_space import ActionSpace
...
class GameState:
    ...
    size = PlayerState.size * 2 + action_dim

    character_offsets = np.array([PlayerState.onehot_offsets['character'] + PlayerState.size * i for i in range(2)])
    action_state_offsets = np.array([PlayerState.onehot_offsets['action_state'] + PlayerState.size * i for i in range(2)])
    action_offset = PlayerState.size * 2

    augmentation = np.array(
        ['action_%d' % i for i in range(action_dim)])

    indexes = {
        key: i for i, key in enumerate(np.concatenate([PlayerState.get(p) for p in range(2)]+[augmentation], axis=0))
    }


    scales = np.concatenate([np.tile(PlayerState.scales, 2), np.ones(action_dim, dtype=np.float32)], axis=0)

    stock_indexes = np.array( [indexes['p0_stocks'], indexes['p1_stocks']], dtype=np.int32)
    ...
    def __init__(self, mw_path, instance_id, test=False):
        ...
        self.frame = 0
        self.chars = None
        self.types = np.zeros(2, dtype=np.int32)

        self.state = np.copy(self._data_placeholder)


        # for one_hot cleaning
        self.action_states_ = np.zeros(2, dtype=np.int32)
        self.characters_ = np.zeros(2, dtype=np.int32)
        self.action_ = 0
        self.manager = StateManager(self, test)
        self.mw = MemoryWatcherZMQ(mw_path, instance_id)
        self.mw_path = mw_path

        self.team_stocks = np.array([4,4], dtype=np.int32)

        self.write_locations()


    def update_players(self, players):
        self.chars = [p for p in players]
    ...
    def action(self):
        return self.state[PlayerState.size*2:]
    ...
    def compute_stocks_left(self):
        ...
        self.team_stocks[:] = self.state[self.stock_indexes]
    ...
    def get(self, target, player_index, action):
        ...
        if player_index == 0:
            ...
            target[:] = np.concatenate([self.player_state(player_index), self.player_state(0)])
